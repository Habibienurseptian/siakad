<!-- Modal Tambah Guru khusus sekolah ini -->
<div id="modalGuru-{{ $sekolah->id }}" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        <!-- Header -->
        <div class="flex justify-between items-center px-6 py-4 border-b">
            <h2 class="text-lg font-bold text-gray-800">Tambah Akun Guru - {{ $sekolah->nama }}</h2>
        </div>

        <!-- Body -->
        <div class="px-6 py-4">
            <form action="{{ route('admin.guru.store') }}" method="POST" id="formTambahGuru-{{ $sekolah->id }}" class="space-y-4">
                @csrf
                ...
                <div class="flex justify-end gap-3 px-6 py-4 border-t">
                    <button type="button" onclick="document.getElementById('modalGuru-{{ $sekolah->id }}').classList.add('hidden')" 
